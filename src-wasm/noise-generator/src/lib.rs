...
#[wasm_bindgen]
impl PerlinNoiseGrid {
    pub fn new(width: u32, height: u32) -> PerlinNoiseGrid {
        PerlinNoiseGrid {
            width,
            height,
            cells: Vec::new(),
...
impl Marcher {
    ...
    pub fn update(&mut self, isovalue: f64, width: usize, height: usize, cells: Vec<f64>) {
        self.width = width;
        self.height = height;
        self.cells.resize(self.width * self.height, 0);
        for y in 1..height {
            for x in 1..width {
                let mut cell = (cells[(y - 1) * width + (x - 1)] > isovalue) as i8;
                cell = cell << 1 | (cells[(y - 1) * width + x] > isovalue) as i8;
                cell = cell << 1 | (cells[y * width + x] > isovalue) as i8;
                cell = cell << 1 | (cells[y * width + (x - 1)] > isovalue) as i8;
