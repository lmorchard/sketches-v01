...
async function main() {
  ...
  spawnRandomCells(world, LIFE_GRID_WIDTH * 5);
...
function spawnRandomCells(world, count = LIFE_GRID_WIDTH * 5) {
  for (let idx = 0; idx < count; idx++) {
    CellEntity.spawnRandom(world);
  }
}
...
class CellEntity extends BaseEntityProxy {
  ...
  static spawnRandom(world) {
    ...
    const cell = this.spawn(world, {
      Cell: { alive: true, zombie: false, presence: Math.random() * 0.5, x, y },
    });
  ...
  return (options = {}) =>
    (world) => {
      ...
      for (const eid of cellQuery(world)) {
        ...
        liveNeighbors.push(...scanResult.liveNeighbors);

        if (!Cell.zombie[eid]) {
          // Live cells behave according to the usual Game of Life rules
          ...
          // Any live neighbors nearby have a chance to be infected
          for (const liveEid of liveNeighbors) {
            if (Math.random() < CHANCE_INFECTION) {
              infections.add(liveEid);
            }
          }
        }
      }

      // Drop presence slightly for survivors so they pulse.
      for (const eid of survivals) {
        if (Cell.presence[eid] === 1.0)
          Cell.presence[eid] = 0.2 + Math.random() * 0.4;
      }

      // Apply infections to cells as appropriate.
      for (const eid of infections) {
        Cell.zombie[eid] = true;
      }

      // Apply deaths to cells as appropriate.
      for (const eid of deaths) {
        if (!Cell.zombie[eid] && Math.random() < CHANCE_UNDEATH) {
          // 10% chance of undeath when the living die
          Cell.zombie[eid] = true;
        } else {
      ...
      for (let idx = 0; idx < births.length; idx += 2) {
        const x = births[idx];
        ...
        const newCell = CellEntity.spawn(world, {
          Cell: { alive: true, zombie: false, presence, x, y },
        });
  ...
  return (world, scanX, scanY) => {
    ...
    for (let x = scanX - 1; x <= scanX + 1; x++) {
      for (let y = scanY - 1; y <= scanY + 1; y++) {
        ...
        if (Cell.alive[eid] && !Cell.zombie[eid]) {
          result.liveNeighborCount++;
          result.liveNeighbors.push(eid);
        } else {
  ...
  for (let x = 0; x < LIFE_GRID_WIDTH; x++) {
    for (let y = 0; y < LIFE_GRID_HEIGHT; y++) {
      ...
      if (cell.presence > 0.0) {
        ...
        const hue = cell.zombie ? HUE_ZOMBIE : HUE_LIFE;
...
function setupTwiddles({
  ...
  f.addInput(lifeOptions, "autospawnPeriod", { min: 0.5, max: 10 });
